...
public class AdvancedAlgorithms {
    ...
    static final class SuffixArray {
        ...
        void buildSA(String s) {
            ...
            // We must sort an Integer[] to use a Comparator
            ...
            for (int k = 1; k < n; k <<= 1) {
                ...
                // copy sorted indices back to primitive sa
                for (int i = 0; i < n; i++) {
                    sa[i] = sai[i];
                }
                ...
                for (int i = 1; i < n; i++) {
                    int prev = sa[i - 1];
                    int curr = sa[i];
                    boolean diff = (r[prev] != r[curr])
                            || ((prev + K < n ? r[prev + K] : -1) != (curr + K < n ? r[curr + K] : -1));
                ...
                // If all ranks are unique, we can stop early
                if (r[sa[n - 1]] == n - 1) {
                    break;
                }
            }
    ...
    static class AStar {
        ...
        static long[] aStar(Graph g, int src, int target, LongUnaryOperator heuristic) {
            long[] gscore = new long[g.n];
            Arrays.fill(gscore, Long.MAX_VALUE / 4);
            gscore[src] = 0;
            long[] fscore = new long[g.n];
            Arrays.fill(fscore, Long.MAX_VALUE / 4);
    ...
    static class Dinic {
        ...
        long maxflow(int s, int t) {
            ...
            while (bfs()) {
                it = new int[n];
                long pushed;
                while ((pushed = dfs(s, Long.MAX_VALUE / 4)) > 0) {
                    flow += pushed;
                }
            }
            return flow;
    ...
    static class FFT {
        ...
        static long[] multiply(long[] a, long[] b) {
            ...
            for (int i = 0; i < n; i++) {
                fa[i] = new Complex(i < n1 ? a[i] : 0, 0);
            }
            Complex[] fb = new Complex[n];
            for (int i = 0; i < n; i++) {
                fb[i] = new Complex(i < n2 ? b[i] : 0, 0);
            }
            fft(fa, false);
            fft(fb, false);
            for (int i = 0; i < n; i++) {
    ...
    static int tspBitmask(int[][] dist) {
        ...
        for (int mask = 1; mask < FULL; mask++) {
            for (int u = 0; u < n; u++) {
                ...
                if (dp[mask][u] >= Integer.MAX_VALUE / 8) {
                    continue;
                }
                for (int v = 0; v < n; v++) {
                    if ((mask & (1 << v)) != 0) {
    ...
    public static void main(String[] args) {
        ...
        if (maxL > 0) {
            pr("Longest repeated substring in '" + t + "' is '" + t.substring(idx, idx + maxL) + "'"); 
        }else {
            pr("No repeated substring found in '" + t + "'");
        }
